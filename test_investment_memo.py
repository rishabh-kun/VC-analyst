"""
Pipeline Test: Full 6-Agent Pipeline
Startup Research Agent -> Founder Evaluation Agent -> Market Analysis Agent -> Financial Analysis Agent -> Risk Assessment Agent -> Investment Memo Agent
Input: OpenAI
"""
...
from agents.investment_memo_agent import InvestmentMemoAgent


def run_pipeline():
    startup_input = "OpenAI"
    print(f"Starting Complete 6-Agent Pipeline for Input: '{startup_input}'\n")
    ...
    print("\n" + "=" * 70)
    print("STEP 6: INVESTMENT MEMO AGENT OUTPUT")
    print("=" * 70)
    memo_agent = InvestmentMemoAgent()
    memo_output = memo_agent.run(
        research_output=research_output,
        founder_output=founder_output,
        market_output=market_output,
        financial_output=financial_output,
    ...
    print(json.dumps(memo_output.model_dump(), indent=2))
    print("=" * 70)

    # Validations matching required output schema
    assert memo_output.status == "SUCCESS"
    assert 0.0 <= memo_output.overall_investment_score <= 10.0
    assert memo_output.recommendation in ("Strong Invest", "Invest", "Watch", "Do Not Invest")
    assert 0 <= memo_output.confidence_score <= 100
    assert len(memo_output.executive_summary) > 0
    assert len(memo_output.reasoning) > 0

    print("\n[SUCCESS] Investment Memo Agent 6-agent pipeline test passed successfully!")
