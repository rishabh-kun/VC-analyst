"""
Pipeline Test: 4-Agent Pipeline
Startup Research Agent -> Founder Evaluation Agent -> Market Analysis Agent -> Financial Analysis Agent
Input: OpenAI
"""
...
def run_pipeline():
    ...
    print(f"Starting 4-Agent Pipeline for Input: '{startup_input}'\n")
    ...
    # Validations against required schema
    assert financial_output.status == "SUCCESS"
    assert 0.0 <= financial_output.financial_score <= 10.0
    assert financial_output.company_type in ("Public", "Private")
    assert len(financial_output.financial_strengths) > 0

    print("\n[SUCCESS] Financial Analysis Agent 4-agent pipeline test passed successfully!")
